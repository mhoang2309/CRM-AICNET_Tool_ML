...
class TranscriptListFetcher(object):
    ...
    def fetch(self, video_id, json3=False):
        captions_json = self._extract_captions_json(self._fetch_video_html(video_id), video_id)
        if json3:
            captions_json = self.json3(captions_json)
    ...
    def json3(self, captions_json):
        for caption in captions_json['captionTracks']:
            baseUrl = caption.get("baseUrl") 
            if baseUrl:
                caption["baseUrl"] = baseUrl + "&fmt=json3"
                break
...
class Transcript(object):
    ...
    def fetch(self, preserve_formatting=False, json3=False):
        """
        ...
        return _TranscriptParser(preserve_formatting=preserve_formatting).parse(
            _raise_http_errors(response, self.video_id).text, json3=json3
        )
...
class _TranscriptParser(object):
    ...
    def parse(self, plain_data, json3):
        if json3:
            events = json.loads(plain_data).get("events", None)
            if events:
                transcript_data = []
                ...
                for event in events:
                    ...
                    if segs and isinstance(segs, list):
                        ...
                        for seg in segs:
                            utf8 = seg.get("utf8")
                            if pattern.search(utf8) or utf8 in ["\n","\t"]:
                                break
                            text += utf8
                            segs_data.append({
                                "offsetMs": seg.get("tOffsetMs", 0),
                                "text": utf8
                            })
                        if segs_data:
...
class TranscriptList(object):
    ...
    def find_transcript(self, language_codes) -> Transcript:
        """
